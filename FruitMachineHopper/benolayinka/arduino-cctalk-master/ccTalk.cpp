...
void ccTalk::device_reset(){
  cctsend(reset_device, deviceID, 0); //ben added because credit buffer stays if power isn't reset
  while (RX_state < RXcomplete) {
    ccTalkReceive(); // wait for an answer or error
  }
...
void ccTalk::device_init(){
  ...
  for (unsigned char i = 1; i < 17; i++) {
    ...
    cctsend(request_coin_id, deviceID, 1, i);
    while (RX_state < RXcomplete) {
      ccTalkReceive();
    }
    if (RX_state != RXcomplete) {
  ...
  cctsend(modify_inhibit_status, deviceID, 2, 255, 255); //just enable all channels
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
  ...
  cctsend(modify_master_inhibit_status, deviceID, 1, 1);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
char * ccTalk::get_master_inhibit(){
  ...
  cctsend(request_master_inhibit_status, deviceID, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
void ccTalk::master_inhibit_on(){
  //Serial.println("Setting master inhibit");
  cctsend(modify_master_inhibit_status, deviceID, 1, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
char * ccTalk::get_inhibit(){
  ...
  cctsend(request_inhibit_status, deviceID, 0);
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
void ccTalk::inhibit_on(){
  cctsend(modify_inhibit_status, deviceID, 2, 0, 0); //just disable all channels
  while (RX_state < RXcomplete) {
    ccTalkReceive();
  }
  // if (RX_state != RXcomplete) //error handling
...
int ccTalk::read_credit(){
  cctsend(read_buffered_credit_or_error_codes, deviceID, 0);
    while (RX_state < RXcomplete) {
      ccTalkReceive();
    }
    if (RX_state != RXcomplete) {
